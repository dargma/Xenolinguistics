...
순방향 Diffusion은 3.81까지 내려가나 역방향은 ~10.6에서 정체(이 괴리의 해석은 §5).
...
**Khalani** (학습 44 / 평가 11, epochs 20, `outputs/khalani[_kha2en]_ft_eval_*.json`). 셀 = **AR / Diffusion**, 굵게 = 우세.
...
전체 300문장을 재생성해 측정한 결과(`outputs/analysis_klingon.json`), 두 모델의 차이는 대부분
**Diffusion이 정답보다 길게 생성한다(적절한 곳에서 일찍 멈추지 못한다)**는 점으로 설명된다.

![analysis](figures/2026-05-29-analysis.png)

**(A) 생성 길이** — 두 모델 모두 EOS가 나오면 멈추지만(§2.3), 멈추는 지점이 다르다. AR은 정답 길이쯤에서
멈춰 출력/정답 길이비가 ~1.0(순 0.95·역 1.01)인 반면, Diffusion은 EOS를 늦게 놓아 정답보다 길게 생성한다 —
특히 낯선 Klingon을 만드는 순방향에서 **1.63배**, 일부 입력(2%)은 `, we have 1,2,3…`처럼 반복 붕괴한다.
이렇게 정답을 넘겨 생성한 잉여 토큰이 틀리거나 반복이라 n-gram 정밀도를 깎고, 순방향 BLEU가 2.55까지
떨어진 주원인이 된다. AR이 순방향 chrF·BLEU에서 앞선 것도 이 **멈추는 타이밍(길이 통제)** 차이다.

**(B) 문장별 승부** — 문장 하나하나의 chrF로 누가 이기는지 세어 보면, 출력이 영어인 역방향에선
Diffusion이 152 대 110으로 분명히 앞선다(평균 +4.2 chrF). 순방향은 문장별로는 거의 호각이다
(Diff 139 / AR 129, 평균 차 +0.4) — 즉 *대부분 문장에선 Diffusion도 대등*하지만, 소수의 과생성·붕괴
문장이 코퍼스 집계(특히 BLEU)를 끌어내려 **§3.1의 코퍼스 점수에선 AR이 앞선다**((A)와 같은 원인).
정리하면, 출력이 모델이 **이미 아는 언어(영어)**일 때 양방향 디코딩의 강점이 드러나고, 낯선 인공어를
만들 땐 그 강점이 소수의 길이 폭주로 상쇄된다.

이런 어려움의 근원은 데이터에 있다 — Klingon은 같은 뜻을 영어보다 토큰을 많이 쓰고, 단어 하나가
토크나이저에서 여러 조각으로 쪼개진다(미지 어휘):

| | 영어 대비 토큰 수 | 단어당 subword 조각 |
|---|:---:|:---:|
| Klingon | ×1.74 | 3.66 |
| Khalani | ×1.39 | — |

**다음 (가변)**: Diffusion 디코딩 ablation(threshold 0.9→0.95, block 32→16)으로 과생성·붕괴가
*디코딩 설정* 문제인지 *아키텍처* 한계인지 분리.
