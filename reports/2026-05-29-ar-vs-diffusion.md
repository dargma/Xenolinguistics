...
전체 300문장을 재생성해 생성 길이·실패 유형·문장별 승부를 측정했다 (`outputs/analysis_klingon.json`).

**① Diffusion은 빈칸을 과하게 채운다.**
- *왜*: Diffusion 순방향 BLEU가 2.55로 무너진 원인을 보려고.
- *결과*: 출력/정답 길이비 — AR은 두 방향 모두 ~1.0(순 0.95·역 1.01)이나 Diffusion은 과생성(순 **1.63**·역 1.24). 순방향 Diffusion은 반복 붕괴도 2%(`, we have 1,2,3…`).
- *인사이트*: **AR의 순방향 우세는 '길이 통제'에서 온다.** 과생성·붕괴가 n-gram 정밀도(BLEU)를 깎는다.

**② 익숙한 언어(영어) 출력에선 Diffusion이 앞선다.**
- *왜*: 코퍼스 평균 뒤에 가린 문장별 승부를 보려고.
- *결과*: 문장별 chrF 승부 — 역방향은 Diffusion 152승 / AR 110승(평균 +4.2), 순방향은 호각(Diff 139 / AR 129).
- *인사이트*: 출력이 모델이 이미 아는 영어일 때 Diffusion의 양방향 디코딩이 더 유창하다. 낯선 Klingon을 만들 땐 그 이점이 과생성으로 뒤집힌다.

**배경 — 인공어는 왜 어려운가.** Klingon은 영어보다 토큰 1.74배, 단어 하나가 평균 3.66 subword로 쪼개진다(미지 어휘). 모델이 낯선 다중 토큰을 만들어야 하는 부담이 순방향 난이도의 근원이다.

**한 줄 요약**: *Diffusion은 빈칸을 넉넉히 채운다 — 아는 영어를 낼 땐 유창함으로, 낯선 인공어를 낼 땐 과생성·붕괴로 나타난다.*

**다음 (가변)**: Diffusion 디코딩 ablation(threshold 0.9→0.95, block 32→16)으로 과생성·붕괴가 *디코딩 설정* 문제인지 *아키텍처* 한계인지 분리.
