...
**Diffusion이 빈칸을 과하게 채우는 성향**으로 설명된다.

![analysis](figures/2026-05-29-analysis.png)

**(A) 생성 길이** — AR은 어느 방향에서든 정답과 거의 같은 길이(0.95·1.01)에서 멈추지만, Diffusion은 더
길게 생성한다. 특히 낯선 Klingon을 만드는 순방향에서 정답의 **1.63배**로 부풀고, 일부 입력(2%)은
`, we have 1,2,3…`처럼 반복 붕괴한다. 순방향 BLEU가 2.55까지 떨어진 것도, AR이 순방향 chrF·BLEU에서
앞선 것도 이 **길이 통제 차이**가 주원인이다.

**(B) 문장별 승부** — 코퍼스 평균을 문장 단위로 풀면, 출력이 영어인 역방향에서 Diffusion이 152 대 110으로
분명히 앞서고(평균 +4.2 chrF), 인공어를 만드는 순방향은 호각이다(139 대 129). 출력이 모델이 **이미 아는
언어(영어)**일 때 양방향 디코딩의 강점이 드러나고, 낯선 인공어를 만들 땐 그 강점이 과생성으로 상쇄된다.
