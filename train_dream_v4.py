"""
Dream-7B v4 — Ablation experiments for root cause analysis
Key changes from v2 (train_dream_official.py):
  - target_modules: all-linear (official config)
  - lr: 2e-6 (official run script)
  - time_reweighting: cart (official run script)
  - max_length: configurable (default 128 for efficiency)
"""
...
# ============================================================
# 1. q_sample — official gen_utils.py
# ============================================================
def q_sample(
    input_ids,
    maskable_mask,
...
# ============================================================
# 2. Context-Adaptive Reweighting (CART) — official
# ============================================================
...
# ============================================================
# 3. Dataset — same as official SFTDataset
# ============================================================
    ...
    def __getitem__(self, idx):
        ...
        if seq_len < self.max_length:
            ...
            attention_mask = torch.cat([
                attention_mask,
                torch.ones(pad_len, dtype=attention_mask.dtype)  # official: pad also attn=1
            ])
...
# ============================================================
# 4. Loss computation — official _compute_loss_and_backward() port
# ============================================================
def compute_dream_loss(
    model, batch, mask_token_id, vocab_size,
    pad_eos_token_id=None, time_reweighting="cart",
    token_reweighting=False, alpha=0.25, gamma=2.0,
    cart_p=0.1, treat_eos_as_one=False,
):
    ...
    # Dream logit shift (self-position prediction)
...
def train(args):
    print(f"{'='*60}")
    print(f"Dream-7B v4 Ablation Experiment")
    ...
    # ---- Data ----
    print(f"\n[3/5] Loading data: {args.train_file} (max_length={args.max_length})")
    ...
    # Check actual sequence statistics
    sample = train_ds[0]
    real_tokens = sample["attention_mask"].sum().item()
    resp_tokens = sample["loss_mask"].sum().item()
    print(f"  Sample seq: {sample['input_ids'].shape[0]} total, ~{real_tokens} real, ~{resp_tokens} response")
    ...
    if train_log:
        ax.plot([d["global_step"] for d in train_log], [d["loss"] for d in train_log],
                label="Train Loss", alpha=0.7)
    if val_log:
        ax.plot([d["global_step"] for d in val_log], [d["val_loss"] for d in val_log],
                label="Val Loss", linestyle="--", marker="o")
    ax.set_xlabel("Step")
    ax.set_ylabel("Loss")
    title_parts = [f"Dream v4 | {args.target_modules} | lr={args.lr} | {args.time_reweighting}"]
    ax.set_title(title_parts[0])
    ax.legend()
    ax.grid(True, alpha=0.3)
    fig.savefig(os.path.join(args.output_dir, "loss_curve.png"), dpi=150, bbox_inches="tight")
    plt.close()

    # ---- Meta ----
    meta = {
        "model": args.model_id,
        "method": "dream_v4_ablation",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Dream-7B v4 Ablation")

    # Experiment
    parser.add_argument("--experiment_name", default="exp1_official_config")
    ...
    parser.add_argument("--target_modules", default="all-linear",
                        help="'all-linear' or comma-separated like 'q_proj,v_proj,k_proj,o_proj'")

    # Data
    parser.add_argument("--train_file", default="data/train_10k.jsonl")
    parser.add_argument("--val_file", default="data/val_10k.jsonl")
    parser.add_argument("--max_length", type=int, default=128)

    # Training (official run script defaults)
    ...
    # Diffusion loss (official run script defaults)
    parser.add_argument("--time_reweighting", default="cart",
                        choices=["original", "linear", "cart", "none"])
    parser.add_argument("--token_reweighting", action="store_true")
    parser.add_argument("--treat_eos_as_one", action="store_true")

    # Output
    parser.add_argument("--output_dir", default="outputs/dream_v4_exp1")
